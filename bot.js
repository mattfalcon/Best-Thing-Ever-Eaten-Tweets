...
client.get('search/tweets', {q: "LivePD", count: 30}, function(error, tweets, response) {
  ...
  client.post('statuses/update', {status: "RT @" + selected_tweet.screen_name + " " + selected_tweet.text + "  Tweet Frm: " + selected_tweet.location + " whatcha want, whatcha gonna do @Stop_Pre"}, function(error, tweet, response) {
    if (!error) {
      console.log(tweet);
    }
    }); 
